...
exports.createNotification = async ({ recipient, type, title, message, link, meta }) => {
  try {
    return await Notification.create({ recipient, type, title, message, link, meta });
  } catch (err) {
...
exports.broadcastNotification = async (recipientIds, payload) => {
  try {
    ...
    logger.info(SERVICE, "Broadcast sent", { count: recipientIds.length });
  } catch (err) {
...
exports.deleteOldNotifications = async () => {
  try {
    ...
    if (result.deletedCount > 0)
      logger.info(SERVICE, `Cleanup: deleted ${result.deletedCount} old notifications`);
  } catch (err) {
