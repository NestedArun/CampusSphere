...
exports.getExams = async (req, res) => {
  try {
    ...
    const exams  = await Exam.find(filter).populate("createdBy", "name").sort({ updatedAt: -1 });
...
exports.createExam = async (req, res) => {
  try {
    ...
    if (!subject || !department)
      return res.status(400).json({ success: false, message: "Subject and department required" });
    const exam = await Exam.create({ subject, department, createdBy: req.user._id, sections: [] });
...
exports.updateMarks = async (req, res) => {
  try {
    ...
    for (const { studentId, score, remarks } of (req.body.marks || [])) {
      const entry = section.marks.find(m => m.student.toString() === studentId);
      if (!entry) continue;
      ...
      if (score !== undefined && score !== oldScore) {
        await notifService.createNotification({
          ...
          link:      "/marks",
          meta:      { examId: exam._id, sectionId: section._id, score, maxScore: section.maxScore },
        });
      }
    }
...
exports.getExamDetail = async (req, res) => {
  try {
    const exam = await Exam.findById(req.params.id)
      .populate("sections.marks.student", "name studentId department");
...
exports.deleteSection = async (req, res) => {
  try {
    const exam = await Exam.findById(req.params.id);
    if (!exam) return res.status(404).json({ success: false, message: "Not found" });
    exam.sections = exam.sections.filter(s => s._id.toString() !== req.params.sectionId);
    await exam.save();
  ...
  try {
    ...
    const result = exams.map(exam => ({
      ...
      sections: exam.sections.map(sec => {
        const entry = sec.marks.find(m => m.student?.toString() === req.user._id.toString());
        return {
          _id:      sec._id,
          title:    sec.title,
