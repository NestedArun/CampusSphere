...
import {
  ...
  Building, MapPin, Bus, ClipboardList, BarChart2, FileText,
  Users, GraduationCap, LogOut, ChevronRight,
} from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    <div className="w-56 h-screen bg-primary border-r border-white/8 flex flex-col shrink-0 overflow-y-auto">
      {/* Logo */}
      ...
      <div className="px-2 py-3 border-t border-white/8">
        <div className="flex items-center gap-2.5 px-3 py-2 rounded-xl hover:bg-white/5 transition group">
          ...
          <button onClick={() => { logout(); navigate("/"); }} title="Logout"
            className="text-soft hover:text-red-400 transition shrink-0">
            <LogOut size={14} />
          </button>
        </div>
      </div>
